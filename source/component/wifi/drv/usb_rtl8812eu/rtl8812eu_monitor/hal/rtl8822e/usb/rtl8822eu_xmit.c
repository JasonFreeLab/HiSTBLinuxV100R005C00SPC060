...
static void update_txdesc_h2c_pkt(struct xmit_frame *pxmitframe, u8 *pmem, s32 sz)
{
	...
	SET_TX_DESC_TXPKTSIZE_8822E(ptxdesc, sz);
	SET_TX_DESC_QSEL_8822E(ptxdesc, HALMAC_TXDESC_QSEL_H2C_CMD);
	rtl8822e_cal_txdesc_chksum(padapter, ptxdesc);
	rtl8822e_dbg_dump_tx_desc(padapter, pxmitframe->frame_tag, ptxdesc);
}
...
static s32 update_txdesc(struct xmit_frame *pxmitframe, u8 *pmem, s32 sz, u8 bagg_pkt)
{
	...
	/* offset 0 */
	SET_TX_DESC_LS_8822E(ptxdesc, 1);
	...
	SET_TX_DESC_OFFSET_8822E(ptxdesc, offset);

	if (bmcst)
		SET_TX_DESC_BMC_8822E(ptxdesc, 1);
	...
	if (pxmitframe->pkt_offset > 0)
		SET_TX_DESC_PKT_OFFSET_8822E(ptxdesc, pxmitframe->pkt_offset);

	SET_TX_DESC_MACID_8822E(ptxdesc, pattrib->mac_id);
	SET_TX_DESC_RATE_ID_8822E(ptxdesc, pattrib->raid);

	SET_TX_DESC_QSEL_8822E(ptxdesc,  pattrib->qsel);
	...
	if (pattrib->inject != 0xa5) {
		if (!pattrib->qos_en) {
			...
			SET_TX_DESC_DISQSELSEQ_8822E(ptxdesc, 1);
			SET_TX_DESC_EN_HWSEQ_8822E(ptxdesc, 1);/* Hw set sequence number */
			SET_TX_DESC_HW_SSN_SEL_8822E(ptxdesc, pattrib->hw_ssn_sel);
			SET_TX_DESC_EN_HWEXSEQ_8822E(ptxdesc, 0);
		} else {
			SET_TX_DESC_SW_SEQ_8822E(ptxdesc, pattrib->seqnum);
		}
	}
	...
	if (pattrib->inject == 0xa5) {
		...
		SET_TX_DESC_EN_HWSEQ_8822E(ptxdesc, 0); /* Hw do not set sequence number */
		SET_TX_DESC_SW_SEQ_8822E(ptxdesc, pattrib->seqnum); /* Copy inject sequence number to TxDesc */

		SET_TX_DESC_RTY_LMT_EN_8822E(ptxdesc, 1);

		if (pattrib->retry_ctrl == _TRUE) {
			SET_TX_DESC_RTS_DATA_RTY_LMT_8822E(ptxdesc, 6); // todo: idk if it's the correct api
		} else {
			SET_TX_DESC_RTS_DATA_RTY_LMT_8822E(ptxdesc, 0);
		}
		if (pattrib->sgi == _TRUE) {
			SET_TX_DESC_DATA_SHORT_8822E(ptxdesc, 1);
		} else {
			SET_TX_DESC_DATA_SHORT_8822E(ptxdesc, 0);
		}

		// ?
		//SET_TX_DESC_DISABLE_FB_8812(ptxdesc, 1); // svpcom: ?
		SET_TX_DESC_DISDATAFB_8822E(ptxdesc, 1);   // ?
		SET_TX_DESC_DISRTSFB_8822E(ptxdesc, 1);	   // See issue #2
		...
		SET_TX_DESC_DATA_STBC_8822E(ptxdesc, pattrib->stbc & 3);
		SET_TX_DESC_DATA_BW_8822E(ptxdesc, pattrib->bwmode); // 0 - 20 MHz, 1 - 40 MHz, 2 - 80 MHz

	} else if ((pxmitframe->frame_tag & 0x0f) == DATA_FRAMETAG) {
		/* RTW_INFO("pxmitframe->frame_tag == DATA_FRAMETAG\n");	*/
		rtl8822e_fill_txdesc_sectype(pattrib, ptxdesc);
#ifdef CONFIG_TCP_CSUM_OFFLOAD_TX
		if (pattrib->hw_csum == 1) {
			int offset = 48 + pxmitframe->pkt_offset*8 + 24;

			SET_TX_DESC_OFFSET_8822E(ptxdesc, offset);
			SET_TX_DESC_CHK_EN_8822E(ptxdesc, 1);
			SET_TX_DESC_WHEADER_LEN_8822E(ptxdesc, (pattrib->hdrlen + pattrib->iv_len + XATTRIB_GET_MCTRL_LEN(pattrib))>>1);
		}
#endif
		...
		if (pxmitframe->agg_num > 1) {
			...
			SET_TX_DESC_DMA_TXAGG_NUM_8822E(ptxdesc, pxmitframe->agg_num);
		}
#endif /* CONFIG_USB_TX_AGGREGATION */

		rtl8822e_fill_txdesc_vcs(padapter, pattrib, ptxdesc);

		if (bmcst)
			rtl8822e_fill_txdesc_force_bmc_camid(pattrib, ptxdesc);
		...
		if ((pattrib->ether_type != 0x888e) &&
			...
			if (pattrib->ampdu_en == _TRUE) {
				SET_TX_DESC_AGG_EN_8822E(ptxdesc, 1);
				SET_TX_DESC_MAX_AGG_NUM_8822E(ptxdesc, 0x1f);
				/* Set A-MPDU aggregation */
				SET_TX_DESC_AMPDU_DENSITY_8822E(ptxdesc, pattrib->ampdu_spacing);
			} else
				SET_TX_DESC_BK_8822E(ptxdesc, 1);

			rtl8822e_fill_txdesc_phy(padapter, pattrib, ptxdesc);

			/* compatibility for MCC consideration, use pmlmeext->cur_channel */
			if (!bmcst) {
				if (pmlmeext->cur_channel > 14)
					/* for 5G. OFDM 6M */
					SET_TX_DESC_DATA_RTY_LOWEST_RATE_8822E(ptxdesc, 4);
				else
					/* for 2.4G. CCK 1M */
					SET_TX_DESC_DATA_RTY_LOWEST_RATE_8822E(ptxdesc, 0);
			}
			...
			if (pHalData->fw_ractrl == _FALSE) {
				...
				SET_TX_DESC_DATARATE_8822E(ptxdesc, (pHalData->INIDATA_RATE[pattrib->mac_id] & 0x7F));
			}
			if (bmcst) {
				DriverFixedRate = 0x01;
				rtl8822e_fill_txdesc_bmc_tx_rate(pattrib, ptxdesc);
			}
			...
			if (padapter->fix_rate != 0xFF) {
				...
				SET_TX_DESC_DATARATE_8822E(ptxdesc, (padapter->fix_rate & 0x7F));
			...
			if (pattrib->stbc)
				SET_TX_DESC_DATA_STBC_8822E(ptxdesc, 1);

#ifdef CONFIG_WMMPS_STA
			if (pattrib->trigger_frame)
				SET_TX_DESC_TRI_FRAME_8822E (ptxdesc, 1);
			...
			*/
			...
			if((pattrib->ether_type == ETH_P_ARP) &&
				...
				SET_TX_DESC_DATARATE_8822E(ptxdesc, MRateToHwRate(IEEE80211_OFDM_RATE_6MB));
			...
			SET_TX_DESC_DATARATE_8822E(ptxdesc, MRateToHwRate(pmlmeext->tx_rate));
		}
		...
		SET_TX_DESC_RTY_LMT_EN_8822E(ptxdesc, 1);
		if (pattrib->retry_ctrl == _TRUE)
			SET_TX_DESC_RTS_DATA_RTY_LMT_8822E(ptxdesc, 6);
		else
			SET_TX_DESC_RTS_DATA_RTY_LMT_8822E(ptxdesc, 12);

		/* VHT NDPA or HT NDPA Packet for Beamformer. */
		rtl8822e_fill_txdesc_mgnt_bf(pxmitframe, ptxdesc);
#if defined(CONFIG_CHANGE_DTIM_PERIOD) && defined(CONFIG_AP_MODE)
		/* BIT7: enable bit */
		if (pattrib->subtype == WIFI_BEACON && pattrib->dtim_period > 0)
			SET_TX_DESC_GROUP_BIT_IE_OFFSET_8822E(ptxdesc, pattrib->tim_ie_offset | BIT(7));
#endif
#ifdef CONFIG_XMIT_ACK
	...
	rtl8822e_fill_txdesc_bf(pxmitframe, ptxdesc);

	if (DriverFixedRate)
		SWDefineContent |= 0x01;

	SET_TX_DESC_SW_DEFINE_8822E(ptxdesc, SWDefineContent);

	SET_TX_DESC_PORT_ID_8822E(ptxdesc, hw_port);
	SET_TX_DESC_MULTIPLE_PORT_8822E(ptxdesc, hw_port);
 ...
 */
s32 rtl8822eu_xmit_buf_handler(PADAPTER padapter)
{
	PHAL_DATA_TYPE phal;
...
static s32 rtl8822eu_xmitframe_complete(PADAPTER padapter, struct xmit_priv *pxmitpriv, struct xmit_buf *pxmitbuf)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(padapter);
	struct dvobj_priv *pdvobj = adapter_to_dvobj(padapter);
...
#endif
		if (MAX_TX_AGG_PACKET_NUMBER_8822E == pfirstframe->agg_num)
			break;
...
static void rtl8822eu_xmit_tasklet(unsigned long priv)
{
	int ret = _FALSE;
	...
	while (1) {
		...
		ret = rtl8822eu_xmitframe_complete(padapter, pxmitpriv, NULL);
...
s32	rtl8822eu_init_xmit_priv(PADAPTER padapter)
{
	struct xmit_priv *pxmitpriv = &padapter->xmitpriv;
	HAL_DATA_TYPE *pHalData = GET_HAL_DATA(padapter);
	...
	tasklet_init(&pxmitpriv->xmit_tasklet,
		     rtl8822eu_xmit_tasklet,
		     (unsigned long)padapter);
#endif
#ifdef CONFIG_TX_EARLY_MODE
	pHalData->bEarlyModeEnable = padapter->registrypriv.early_mode;
#endif
	rtl8822e_init_xmit_priv(padapter);
	return _SUCCESS;
}

void	rtl8822eu_free_xmit_priv(PADAPTER padapter)
{
	struct xmit_priv *pxmitpriv = &padapter->xmitpriv;
...
s32 rtl8822eu_mgnt_xmit(PADAPTER padapter, struct xmit_frame *pmgntframe)
{
 ...
 */
s32 rtl8822eu_hal_xmit(PADAPTER padapter, struct xmit_frame *pxmitframe)
{
	return pre_xmitframe(padapter, pxmitframe);
}

#ifdef CONFIG_RTW_MGMT_QUEUE
s32 rtl8822eu_hal_mgmt_xmitframe_enqueue(PADAPTER padapter, struct xmit_frame *pxmitframe)
{
	struct xmit_priv *pxmitpriv = &padapter->xmitpriv;
	s32 err;
...
s32 rtl8822eu_hal_xmitframe_enqueue(PADAPTER padapter, struct xmit_frame *pxmitframe)
{
	struct xmit_priv *pxmitpriv = &padapter->xmitpriv;
	s32 err;
...
static void rtl8822eu_hostap_mgnt_xmit_cb(struct urb *urb)
{
#ifdef PLATFORM_LINUX
...
s32 rtl8822eu_hostap_mgnt_xmit_entry(PADAPTER padapter, _pkt *pkt)
{
#ifdef PLATFORM_LINUX
	u16 fc;
	int rc, len, pipe;
